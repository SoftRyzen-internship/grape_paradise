		...
		return (
			<div className={clsx('relative flex flex-col gap-2', className)}>
				...
				<input
					...
					id={id}
					className={clsx(
						'border-b border-stroke bg-transparent py-[10px] font-extralight caret-logo transition placeholder:text-grey placeholder:transition hover:border-logo focus:border-logo focus:outline-0 focus:placeholder:opacity-0 lg:py-4',
						errorMessage && 'border-red/100 hover:border-red focus:border-red',
					)}
				/>
				{errorMessage ? (
					<>
						<span className='absolute bottom-0 left-0 flex translate-y-full pt-1 text-xs/normal font-extralight text-red transition'>
							{errorMessage}
						</span>
						<ErrorIcon
							width={24}
